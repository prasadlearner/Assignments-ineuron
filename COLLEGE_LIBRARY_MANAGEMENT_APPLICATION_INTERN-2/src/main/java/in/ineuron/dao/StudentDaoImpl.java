	...
	public StudentBookTracker getBorrowDateAndSubmDate(String sid, String bid) {
		...
		StudentBookTracker bookTracker = null;
			...
			try {
				connection = JdbcUtil.getJdbcConnection();
				if (connection != null) {	
					pstmt = connection.prepareStatement("SELECT borrowdate,submitdate FROM studentbooktracker WHERE  sid = ? AND bid=?");
				}
				if (pstmt != null) {
					pstmt.setString(1, sid);
	...
	public Book bookSearchInLib(String attribute, String value) {
		...
		Book book = null;
			...
			try {
				...
				//pstmt = connection.prepareStatement( "SELECT * FROM book WHERE bauthor LIKE ? AND btitle LIKE ? AND bcategory LIKE ? ");
